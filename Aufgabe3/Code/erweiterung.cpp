...
double u; int n; // Umfang des Sees, Anzahl der Häuser
vector<double> house; // Liste der Häuser
// maxSize[s][i][j]:
...
vector<vector<double>> maxSize[2];

// Modulorechnung mit double Werten
double mod(double x, double m)
{
    if(x < 0) return m - mod(-x, m);
    else return x - (m * floor(x / m));
}
...
void precalcMaxSize()
{
	...
	for(int s = 0; s < 2; ++s)
	{
		maxSize[s].assign(n, vector<double>(7));
        ...
        for(int d = 0; d < 7; ++d)
        {
            ...
            for(int i = 1; i + wanted <= j; ++i)
            {
                ...
                double val = mod(house[(i+wanted) % n] - house[i], u);
			...
			// Es gilt: size = 2 * G
            maxSize[s][0][d] = 2*q.front().first;

            for(int i = 1; i < n; ++i)
            {
                j = i + sum;
                double val = mod(house[j%n] - house[(j-wanted+n)%n], u);
						...
						for(int p1 = 0; p1 < 2; ++p1)
						{
							for(int p2 = 0; p2 < 2 - p1; ++p2)
							{
								for(int p3 = 0; p3 < 2 - p2 - p1; ++p3)
								{
									...
									double a = maxSize[p1][i][x+5];
									double b = maxSize[p2][(j-1+h2+n)%n][y+5];
									double c = maxSize[p3][(k-1+h3+n)%n][z+5];
									...
									double e2, e3;
									if(h2) e2 = house[j % n];
									else e2 = mod(e1 + min(a, mod(house[j] - e1, u)), u);
									if(h3) e3 = house[k % n];
									else e3 = mod(e2 + min(b, mod(house[k] - e2, u)), u);
									...
									if(mod(house[(j-1+n)%n] - e1, u) >= mod(e2 - e1, u)) continue;
									if(mod(house[(k-1+n)%n] - e2, u) >= mod(e3 - e2, u)) continue;

									// Finale Überprüfung, ob die Abstände der Eisbuden klein genug sind
									if(mod(e2 - e1, u) > a) continue;
									if(mod(e3 - e2, u) > b) continue;
									if(mod(e1 - e3, u) > c) continue;

									// Es wurde eine Lösung gefunden!
									// Sortiere die Positionen und gib sie aus!
									vector<double> ans = {e1, e2, e3};
